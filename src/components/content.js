  ...
  return (
    <div className="h-screen w-screen ">
      <img
        src={imageUrl}
        alt="Ảnh nền"
        className="sm:w-full md:w-full lg:w-full xl:w-full sm:h-[500px] md:h-[500px] lg:h-[500px] xl:h-[500px] "
      ></img>

      <div className="mb-8 border-b-4 ">
        <ul class="list-none p-0 m-0 flex overflow-x-auto">
          <li className={`mr-4 ${activeTab === 1 ? 'font-bold' : ''}`}>
            <Link onClick={() => setActiveTab(1)}>Spresense</Link>
          </li>
          <li className={`mr-4 ${activeTab === 1 ? 'font-bold' : ''}`}>
            <Link onClick={() => setActiveTab(1)}>Product specifications</Link>
          </li>
          <li className={`mr-4 ${activeTab === 2 ? 'font-bold' : ''}`}>
            <Link onClick={() => setActiveTab(2)}>Downloads</Link>
          </li>
          <li className={`mr-4 ${activeTab === 3 ? 'font-bold' : ''}`}>
            <Link onClick={() => setActiveTab(3)}>Development guides</Link>
          </li>
          <li className="mx-4">Contact us</li>
          <li className="mx-4">Support</li>
          <li className="mx-4">News</li>
          <li className="mx-4">Buy Spresense</li>
          <li className="mx-4">Workshops & videos</li>
        </ul>
      </div>
